...
const getRecipesByCookbookId = async ({ cookbook_id }) => {
    const recipes = await db.query(
        `SELECT recipes.id AS recipe_id, chapter_id, recipe_name, owner_id, (SELECT photo_url 
                                                                                from photos
                                                                                WHERE photos.recipe_id = recipes.id                                                                                                                                                           
                                                                                ORDER BY photos.id DESC
                                                                                LIMIT 1) AS recipe_photo
        FROM recipes          
        WHERE cookbook_id = $1
         `,
        [cookbook_id]
...
const getRecipesById = async ({ recipe_id }) => {
    const recipe = await db.query(
        ...
        difficulty_level, recipe_story, (SELECT photo_url 
                                            from photos
                                            WHERE photos.recipe_id = recipes.id                                                                                                                                                           
                                            ORDER BY photos.id DESC
                                            LIMIT 1) AS recipe_photo
